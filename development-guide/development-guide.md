...
- Upload field: Lets users upload files (e.g., certificates, keys) through the setup form. Use `allowed_file_type`(e.g., [".pem", ".json"]) to specify permitted file types and `max_file_size_bytes` to set a file size limit. Please follow this [example](https://github.com/fivetran/fivetran_partner_sdk/blob/2f4f5d401d621db5ea66b70590c8914b2c88b65a/examples/destination_connector/java/src/main/java/destination/DestinationServiceImpl.java#L201). Uploaded files are automatically converted to Base64-encoded strings in the SDK configuration object. You must implement decoding of the Base64 string to reconstruct the uploaded file locally.
