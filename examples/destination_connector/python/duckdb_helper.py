...
class DuckDBHelper:
    ...
    def create_table(self, schema_name, table):
        ...
        for column in table.columns:
            column_def = f'"{column.name}" {self._map_datatype_to_sql(column.type)}'
            column_defs.append(column_def)

        columns_str = ", ".join(column_defs)
        sql = f'CREATE TABLE "{schema_name}"."{table.name}" ({columns_str})'
    ...
    def add_column(self, schema_name, table_name, column):
        ...
        sql = f'ALTER TABLE "{schema_name}"."{table_name}" ADD COLUMN "{column.name}" {self._map_datatype_to_sql(column.type)}'
        self.connection.execute(sql)
    ...
    def drop_column(self, schema_name, table_name, column_name):
        ...
        sql = f'ALTER TABLE "{schema_name}"."{table_name}" DROP COLUMN "{column_name}"'
        self.connection.execute(sql)
    ...
    def rename_column(self, schema_name, table_name, old_name, new_name):
        """Rename a column."""
        sql = f'ALTER TABLE "{schema_name}"."{table_name}" RENAME COLUMN "{old_name}" TO "{new_name}"'
        self.connection.execute(sql)
    ...
    def rename_table(self, schema_name, old_name, new_name):
        """Rename a table."""
        sql = f'ALTER TABLE "{schema_name}"."{old_name}" RENAME TO "{new_name}"'
        self.connection.execute(sql)
    ...
    def copy_table(self, schema_name, from_table, to_table):
        ...
        sql = f'CREATE TABLE "{schema_name}"."{to_table}" AS SELECT * FROM "{schema_name}"."{from_table}"'
        self.connection.execute(sql)
    ...
    def update_column_value(self, schema_name, table_name, column_name, value):
        ...
        sql = f'UPDATE "{schema_name}"."{table_name}" SET "{column_name}" = ?'
