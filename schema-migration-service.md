...
This migration should add a new column with a default value. The `MigrationDetails` object will contain the schema, table and `AddColumnWithDefaultValue` operation holds column name, column type, and the default value to be set for the new column.
```sql
...
This migration should add column to table in history mode. The `MigrationDetails` object will contain the schema, table and the `AddColumnInHistoryMode` operation holds column name, column type,default value, and operation_timestamp.
...
This migration should update the specified column with a new default value. The `MigrationDetails` object will contain the schema, table, and `UpdateColumnValueOperation` operation object will contain the column name and value. 
```sql
  UPDATE <schema.table> SET <column_name> = <new_value>;
```

`NULL` can also be expected to update the column value.  

### RENAME OPERATION

#### RENAME_TABLE
This migration should rename the specified table in the schema. The `MigrationDetails` object will contain the schema, table and `RenameTable` operations contains from_table (old name), and to_table (new name).
```sql
...
#### RENAME_COLUMN
This migration should rename the specified column in the table. The `MigrationDetails` object will contain the schema, table and `RenameColumn` operation will contain from_column(old name) and to_column(new name).
```sql
...
#### COPY_TABLE
This migration should create a new table and copy the data from the `from_table` to `to_table`. The `MigrationDetails` object will contain the schema, table, and the `CopyTable` will contain from_table (old name), and to_table (new name).
Perform the following steps:
1. Create a new table with the new name:
```sql
  CREATE TABLE <schema.to_table> AS SELECT * FROM <schema.from_table>;
...
This migration should copy the table to history mode. The `MigrationDetails` object will contain the schema, table, and the `CopyTableToHistoryMode` field holds from_table (old name), to_table (new name), and soft_deleted_column.
Perform the following steps:
...
```
3. Follow steps in `LEGACY_TO_HISTORY` if `soft_deleted_column` is not null or `LIVE_TO_HISTORY` and migrate this new table to history mode.

#### COPY_COLUMN
This migration should add a new column and copy the data from the `from_column`to `to_column`. The `MigrationDetails` object will contain the schema, table and `CopyColumn` will contain from_column (old name), and to_column (new name).
Perform the following steps:
1. Add the new column with the same type as the old column:
```sql
  ALTER TABLE <schema.table> ADD COLUMN <to_column> <column_type>;
...
#### DROP OPERATION
This migration should drop the table. The `MigrationDetails` object will contain the schema, table and `operation` field will consist of `drop_table` boolean which indicates if the table is top be dropped or `DropColumnInHistoryMode` is present with the column details that needs to be dropped.
...
This migration should drop the history column from the table in history mode. The `MigrationDetails` object will contain the schema, table and `DropColumnInHistoryMode` operation will hold column, and operation_timestamp.
Perform the following steps:
...
This migration should perform migration from one type to another The `MigrationDetails` object will contain the schema, table and based on the `TableSyncModeMigrationType` field perform the migration. 
Additional fields `soft_deleted_column` and `keep_deleted_rows` are used to perform these migrations wherever relevant.
#### LIVE_TO_HISTORY
Perform the following steps:
...
```
2. Set all the records as active and set  _fivetran_start, fivetran_end, and _fivetran_active columns appropriately.
   ```sql
     UPDATE <schema.table>
...
This migration should convert the table to history mode.
Perform the following steps:
1. Add the history mode columns to the table:
```sql
  ALTER TABLE <schema.table> ADD COLUMN _fivetran_start TIMESTAMP,
...
2. Use soft_deleted_column to identify active records and set the values of _fivetran_start, fivetran_end, and _fivetran_active columns appropriately
    ```sql
        UPDATE <schema.table>
        SET 
...
#### HISTORY_TO_LIVE
This migration should convert the table from history mode to live mode.
Perform the following steps:
1. Drop primary key constraint if exists:
    ```sql
      ALTER TABLE <schema.table> DROP CONSTRAINT IF EXISTS <primary_key_constraint>;
    ```
2. If `keep_deleted_rows` is false then drop Deprecated rows ( skip if `keep_deleted_rows` is true):
    ```sql
...
This migration should convert the table from history mode to legacy mode.
Perform the following steps:
1. Drop primary key constraint if exists:
    ```sql
      ALTER TABLE <schema.table> DROP CONSTRAINT IF EXISTS <primary_key_constraint>;
...
4. update `_fivetran_deleted` column based on `_fivetran_active`.
    ```sql
      ...
      SET _fivetran_deleted = CASE 
                              ...
                              END;
    ``` 
   NOTE: also update this to `soft_deleted_column` if it exists and different from `_fivetran_deleted`.
...
#### SOFT_DELETE_TO_LIVE
This migration should convert the table from legacy mode to live mode.
Perform the following steps:
...
#### LIVE_TO_SOFT_DELETE
This migration should convert the table from live mode to legacy mode.
Perform the following steps:
1. Add `_fivetran_deleted` column if it does not exist:
    ...
    ```
2. Update `_fivetran_deleted` column based on the `soft_deleted_column`:
    ```sql
      UPDATE <schema.table>
      SET _fivetran_deleted = 
          CASE WHEN <soft_deleted_column> = TRUE 
          THEN TRUE 
          ELSE FALSE 
          END;
    ```